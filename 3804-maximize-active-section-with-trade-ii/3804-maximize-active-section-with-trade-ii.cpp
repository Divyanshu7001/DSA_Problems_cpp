...
public:
    vector<int> maxActiveSectionsAfterTrade(string s,
        ...
        // if there is only one block of 0s..in that case no 1 will be blocked
        // from both ends so that we can apply any operation in such case...just
        // return a vector sized of queries with all values as no. of 1s found
        // in the string with activeCount
        if (m < 2) {
        ...
        // Pairsum: paisSum[i]=blockSize[i]+blockSize[i+1]
        ...
        // data() to convert the form from vector to arr[]
