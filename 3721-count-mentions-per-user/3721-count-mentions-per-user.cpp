class Solution {
public:
    void applyMessageEvent(vector<string> event, vector<int>& mentionCount,
        ...
        for (string id : ids) {
            ...
            if (id == "ALL") {
                ...
                mentionCount[stoi(id.substr(
                    2))]++; // fetch id value i.e. x from idx (id0, id1 etc)
            }
        }
    }
    ...
    vector<int> countMentions(int numberOfUsers,
        ...
        auto lambda = [](vector<string>& vec1, vector<string>& vec2) {
            ...
            if (t1 == t2) {
                ...
                return vec1[0][1] > vec2[0][1]; // comparing 'O' of "OFFLINE"
                                                // with 'M' of "MESSAGE"
