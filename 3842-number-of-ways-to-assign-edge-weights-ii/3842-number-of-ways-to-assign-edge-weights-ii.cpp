...
public:
    ...
    // filling the first just before the node ancestor
    ...
    // building the other level by level ancestors(Binary lifting table)
    ...
    // Least common ancestor finding..first make them move to same level..then
    // largest jump possibles taken by binary lift jumps Returns number of edges
    // on the path
    int LCA(int u, int v) {
    ...
    // Helper function for both nodes together for case of diff depths
...
// Precomputed powers of 2...for binary lift helpers..we are keeping all the 2
// powers from before to be o(1) for path lengths
